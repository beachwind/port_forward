    ...
    def _launch_cli(self, cli: dict, server: dict, username: str, password: str,
        ...
        try:
            if "putty" in path.lower():
                # ── PuTTY 명령 구성 ─────────────────────────────────
                # 기본: putty -ssh user@host -P port
                cmd_base = [path, "-ssh", f"{username}@{host}", "-P", str(port)]

                if is_key_auth and auth_method == "ppk":
                    # PPK 키 인증: -i keyfile [-pw passphrase]
                    cmd_real = cmd_base + ["-i", key_path]
                    cmd_log  = cmd_base + ["-i", key_path]
                    if key_passphrase:
                        cmd_real += ["-pw", key_passphrase]
                        cmd_log  += ["-pw", "****"]
                else:
                    # 비밀번호 인증: -pw password
                    cmd_real = cmd_base + ["-pw", password]
                    cmd_log  = cmd_base + ["-pw", "****"]

                # init_dir: PuTTY 원격 커맨드(-m 은 파일경로 전용이므로 사용 안 함)
                # putty -ssh user@host -P port -pw pass -t "cd /path && exec bash"
                # → 단, GUI PuTTY는 대화형 세션이므로 -t 옵션 없이 -m 임시파일 방식 사용
                if init_dir:
                    import tempfile as _tf, os as _os
                    # 실행할 원격 커맨드를 임시 스크립트 파일에 저장 후 -m 전달
                    tmp_fd, tmp_script = _tf.mkstemp(suffix=".txt", prefix="putty_cmd_")
                    _os.close(tmp_fd)
                    with open(tmp_script, "w", newline="\n") as f:
                        f.write(f"cd {init_dir}\n")
                    cmd_real += ["-m", tmp_script]
                    cmd_log  += ["-m", f"<tempfile: cd {init_dir}>"]
                    LOG.cmd(cmd_log, category="PUTTY",
                            note=f"auth={auth_method}  init_dir={init_dir}  "
                                 f"script={tmp_script}")
                else:
                    LOG.cmd(cmd_log, category="PUTTY",
                            note=f"auth={auth_method}  init_dir=(기본)")

                subprocess.Popen(cmd_real)
