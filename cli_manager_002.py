...
class CLIManagerApp(tk.Tk):
    ...
    def _show_servers(self, edit=False):
        ...
        cols = ("icon", "name", "host", "port", "username", "auth", "key_file", "init_dir", "description")
        ...
        tree.heading("key_file", text="키 파일")
        tree.heading("init_dir", text="접속 디렉토리")
        tree.heading("description", text="설명")

        tree.column("icon", width=28, anchor="center")
        tree.column("name", width=130)
        tree.column("host", width=150)
        tree.column("port", width=50, anchor="center")
        tree.column("username", width=90)
        tree.column("auth", width=80, anchor="center")
        tree.column("key_file", width=130)
        tree.column("init_dir", width=130)
        tree.column("description", width=160)
        ...
        for srv in self.config_mgr.get_servers():
            auth_method = srv.get("auth_method", "password")
            auth_display = AUTH_ICONS.get(auth_method, "🔑 비밀번호")
            # Key file display: show filename only
            if auth_method == "ppk":
                key_f = Path(srv.get("ppk_path", "")).name
            elif auth_method == "pem":
                key_f = Path(srv.get("pem_path", "")).name
            else:
                key_f = ""
            init_dir = srv.get("init_dir", "")
            tree.insert("", tk.END, values=(
                "🖥️", srv.get("name", ""), srv.get("host", ""),
                srv.get("port", 22), srv.get("username", ""),
                auth_display, key_f, init_dir, srv.get("description", "")
            ))
